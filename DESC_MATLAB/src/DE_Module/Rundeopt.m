function [FVr_x,ECSs] = Rundeopt(S)
%********************************************************************
...
FVr_minbound = log10sig(1)*ones(1,length(nmagg));
FVr_maxbound = log10sig(2)*ones(1,length(nmagg));
...
S.ibin  =   [S.evspec_DE.ibin];
S.fmom  =   [S.evspec_DE.qmomest];
...
[FVr_x,S_y,I_nf,~,ECSs] = deopt('objfun',S_struct,S);
