...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [FVr_bestmem,S_bestval,I_nfeval,vp_model,power] = deopt(fname,S_struct,S)
...
[S_val(1), rms_best]       = feval(fname,FM_pop(I_best_index,:),S_struct,S);

S_bestval = S_val(1);                 % best objective function value so far
I_nfeval  = I_nfeval + 1;

for k=2:I_NP                          % check the remaining members
    [S_val(k), rms_temp]  = feval(fname,FM_pop(k,:),S_struct,S);
    I_nfeval  = I_nfeval + 1;
...
while ((I_iter < I_itermax) && (S_bestval.FVr_oa(1) > F_VTR))
    %if(I_iter == I_itermax-1);keyboard;end
    ...
    for k=1:I_NP
        ...
        [S_tempval rms_temp] = feval(fname,FM_ui(k,:),S_struct,S);   % check cost of competitor
        I_nfeval  = I_nfeval + 1;
    ...
    %power=S_bestval.FVr_oa(1);
    power = rms_best;
    vp_model(I_iter,1:length(FVr_bestmem))=FVr_bestmem;
    ...
    if (I_refresh > 0)
        if ((rem(I_iter,I_refresh) == 0) | I_iter == 1)
            fprintf(1,'Iteration: %d,  Best: %1.12f,  F_weight: %f,  F_CR: %f,  I_NP: %d\n',I_iter,S_bestval.FVr_oa(1),F_weight,F_CR,I_NP);
            %var(FM_pop)
            format long e;
            for n=1:I_D
                fprintf(1,'best(%d) = %g\n',n,FVr_bestmem(n));
